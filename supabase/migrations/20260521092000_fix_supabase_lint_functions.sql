-- Fix function-level lint errors reported by Supabase DB lint.
-- Focus: ambiguous RETURNS TABLE references, anonymous_usage key drift, and
-- process_razorpay_payment type/column compatibility.

-- 1) check_and_reset_daily_limit
...
BEGIN
  IF p_user_id IS NOT NULL THEN
    UPDATE public.subscriptions s
    SET conversions_used = COALESCE(s.conversions_used, 0) + v_increment
    WHERE s.user_id = p_user_id;
    RETURN true;
  END IF;
  ...
  EXECUTE format(
    'UPDATE public.anonymous_usage
     SET conversions_count = COALESCE(conversions_count, 0) + $2
     WHERE %I = $1',
    v_anon_key_col
  )
  USING p_ip_address, v_increment;

  RETURN true;
END;
$$;

-- 3) process_razorpay_payment (numeric + integer overload)
DROP FUNCTION IF EXISTS public.process_razorpay_payment(uuid, uuid, text, text, text, integer, text, text, integer);
DROP FUNCTION IF EXISTS public.process_razorpay_payment(uuid, uuid, text, text, text, numeric, text, text, integer);
...
BEGIN
  RETURN QUERY
  SELECT *
  FROM public.process_razorpay_payment(
    p_user_id,
    p_order_id,
    p_razorpay_payment_id,
    p_razorpay_order_id,
...
GRANT EXECUTE ON FUNCTION public.process_razorpay_payment(uuid, uuid, text, text, text, numeric, text, text, integer) TO service_role;
