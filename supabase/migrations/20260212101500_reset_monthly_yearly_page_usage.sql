-- Reset monthly/yearly page usage each billing period and on renewals

ALTER TABLE public.subscriptions
  ADD COLUMN IF NOT EXISTS pages_used_this_month integer NOT NULL DEFAULT 0;
...
BEGIN
  ...
  IF p_user_id IS NOT NULL THEN
    ...
    IF NOT FOUND THEN
      IF v_has_plan_type THEN
        EXECUTE 'INSERT INTO subscriptions (user_id, plan_type, conversions_used, conversions_limit, last_reset_date, timezone, pages_used_this_month)
                 VALUES ($1, $2, 0, 5, $3, $4, 0)'
        USING p_user_id, 'free', v_today, p_timezone;
        v_plan_type := 'free';
      ELSIF v_has_tier THEN
        EXECUTE 'INSERT INTO subscriptions (user_id, tier, conversions_used, conversions_limit, last_reset_date, timezone, pages_used_this_month)
                 VALUES ($1, $2, 0, 5, $3, $4, 0)'
        USING p_user_id, 'free', v_today, p_timezone;
        v_plan_type := 'free';
      ELSE
        EXECUTE 'INSERT INTO subscriptions (user_id, conversions_used, conversions_limit, last_reset_date, timezone, pages_used_this_month)
                 VALUES ($1, 0, 5, $2, $3, 0)'
