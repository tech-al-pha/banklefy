...
const normalizeTransaction = (raw: Record<string, unknown>): RawTransaction => {
  const normalized = { ...raw } as RawTransaction;

  const description = raw.description ?? raw.narration ?? raw.details ?? raw.memo ?? raw.particulars;
  if (description !== undefined && description !== null) {
    normalized.description = String(description);
  }
  ...
  if (ref !== undefined && ref !== null) {
    normalized.refNumber = typeof ref === 'string' ? ref : String(ref);
  }
