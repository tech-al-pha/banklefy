...
export const Hero = () => {
  ...
  return (
    <section className="relative flex min-h-[70vh] items-center justify-center bg-transparent pb-8 pt-0 md:pt-6">

      <div className="relative z-10 mx-auto w-full max-w-[1400px] px-4 text-left">
        <div className="mx-auto grid max-w-none grid-cols-1 md:grid-cols-[1.1fr_0.9fr] items-center gap-4 md:gap-10 pt-0 sm:pt-1 md:pt-12">
          <div className="flex flex-col items-start space-y-4 sm:space-y-6">
            ...
            <div className="hero-highlight-pill inline-flex max-w-full items-center gap-2 sm:gap-3 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 sm:px-4 sm:py-2 text-[0.48rem] sm:text-xs font-semibold uppercase tracking-[0.08em] sm:tracking-[0.28em] text-primary/80 shadow-[0_0_18px_rgba(255,255,255,0.08)] whitespace-normal overflow-visible leading-[1.2] lg:py-2 lg:px-4 lg:text-xs lg:whitespace-nowrap">
              {t("hero.tagline")}
            </div>

            <p className="hero-subtitle mt-1.5 sm:mt-4 mb-4 sm:mb-12 max-w-3xl text-pretty text-[0.68rem] sm:text-sm md:text-lg italic leading-snug sm:leading-relaxed text-white/80 lg:text-lg lg:leading-relaxed">
              {t("hero.subtitle")}
            </p>

            <div className="hero-cta-group mt-3 sm:mt-8 md:mt-12 flex w-full max-w-2xl flex-row items-stretch justify-start gap-2 sm:gap-4 lg:mt-14 lg:gap-4">
              <Button
                size="lg"
                className="group hero-cta-primary w-[68%] sm:w-auto border-0 bg-primary px-4 sm:px-8 text-[0.64rem] sm:text-base lg:text-base text-primary-foreground shadow-none transition-all duration-300 hover:scale-105 no-glow"
                onClick={() => scrollToId("demo")}
              >
                <Upload className="mr-1.5 h-3 w-3 sm:mr-2 sm:h-5 sm:w-5 lg:h-5 lg:w-5 group-hover:rotate-12 transition-transform" aria-hidden="true" />
                {t('hero.uploadBtn')}
              </Button>
              ...
              <Button
                size="lg"
                variant="outline"
                className="btn-target-glow hero-cta-secondary w-[32%] sm:w-auto border-primary/40 bg-[#141414] px-3 sm:px-8 text-[0.62rem] sm:text-base lg:text-base text-primary shadow-none backdrop-blur-xl transition-all duration-300"
