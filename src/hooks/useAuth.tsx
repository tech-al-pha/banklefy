  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      if (import.meta.env.DEV) {
        if (import.meta.env.DEV) { console.log('Auth state change:', event, session?.user?.email || 'no user'); }
      }
      setSession(session);
      ...
      if (error) {
        ...
        supabase.auth.refreshSession().then(({ data: refreshData, error: refreshError }) => {
          if (refreshError || !refreshData.session) {
            if (import.meta.env.DEV) {
              if (import.meta.env.DEV) { console.log('Session refresh failed, clearing auth state'); }
            }
            setSession(null);
            setUser(null);
          } else {
